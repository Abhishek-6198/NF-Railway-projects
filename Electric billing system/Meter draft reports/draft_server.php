                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            if(substr($row["Qtr_ID"],0,7)==$code){
                                ...
                                array_push($temp,$row["EmpNo"],$row["EmpName"],$row["Qtr_No"]."; ".$type,$row["Prev read"],$row["Current read"],$row["Prev Date"],$row["Current Date"],$row["Unit consumed"],$row["Elec_charge"],$row["Fixed_charge"],$row["Total charge"],$book,$page);
