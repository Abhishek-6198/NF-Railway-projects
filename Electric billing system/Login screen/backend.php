    ...
    if(isset($_POST["input"])){
        if($_POST["input"] == "sign in"){
            if(!$connection)
                ...
                $sql="SELECT * FROM `electric database users` WHERE `User name/no`='".$_POST["username"]."'";
                ...
                if ($result->num_rows == 1) {
                    ...
                    if ($result->num_rows == 1) {
                        ...
                        $sql="SELECT * FROM `employee_master` WHERE `EmpNo`='".$_POST["username"]."'";
                        $result = $con->query($sql);
                        if ($result->num_rows == 1) {
                            while($row = $result->fetch_assoc()) {
                                $name=$row["Name"];
        ...
        elseif($_POST["input"] == "update_records"){
            if(!$connection)
                ...
                foreach($data as $d){
                    ...
                    $sq="SELECT * FROM `employee_master` WHERE  `EmpNo`='".$no."'";
                    $res = $con->query($sq);
                    if ($res->num_rows > 0){
                        ...
                        echo "The employee should be registered in emp master";
                }
            }
        }
