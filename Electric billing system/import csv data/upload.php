        ...
        if($_POST["input"]=="qtr_master(wo id)"){
            ...
            if(!$connection)
                ...
                foreach($data as $d){
                    ...
                    $res = $con->query($sq);
                    if($res->num_rows==0){
                        $stmt = $con->prepare("INSERT INTO `employee_master`(`EmpNo`, `Name`, `Designation`, `Department`,`BillUnit`, `Station`, `Date_of_ret`) 
                                                VALUES (?, ?, ?, ?, ?, ?, ?)");
                        $excel_date = $data1[6]; 
                        $unix_date = ($excel_date - 25569) * 86400;
                        $excel_date = 25569 + ($unix_date / 86400);
                        $unix_date = ($excel_date - 25569) * 86400;
                        $data1[6]= gmdate("d/m/Y", $unix_date);
                        $stmt->bind_param("sssssss", $data1[0],$data1[1],$data1[2],$data1[3],$data1[4],$data1[5],$data1[6]);
                        if($stmt->execute())
                            $count+=1;
                        $stmt->close();
                    }
                    else{
                        $statement = $con->prepare("UPDATE `employee_master` SET `Name`=?, `Designation`=?, `Department`=?, `BillUnit`=?, `Station`=?, `Date_of_ret`=? WHERE `EmpNo`='".$data1[0]."'");
                        $excel_date = $data1[6]; 
                        $unix_date = ($excel_date - 25569) * 86400;
                        $excel_date = 25569 + ($unix_date / 86400);
                        $unix_date = ($excel_date - 25569) * 86400;
                        $data1[6]= gmdate("d/m/Y", $unix_date);
                        $statement->bind_param("ssssss",$data1[1],$data1[2],$data1[3],$data1[4],$data1[5],$data1[6]);
                        if($statement->execute())
                            $count1+=1;
                        $statement->close();
                    }
                }
                echo $count." records inserted".";"." ".$count1." records updated";
            }
        }
    }
    $con->close();
