        ...
        elseif($_POST["input"]=="calculate_charges"){
                ...
                if(!$connection)
                    ...
                    foreach($data as $d){
                        ...
                        $sql="SELECT * FROM `installment_procurement` WHERE `Qtr_ID`='".$qtr_id."'";
                        $result = $con->query($sql);
                        if ($result->num_rows == 0) {
                            $stmt=$con->prepare("INSERT INTO `installment_procurement`(`Qtr_ID`,`Installment`) VALUES (?,?)");
                            $stmt->bind_param("si",$qtr_id,$months);
                            $stmt->execute();
                            $stmt->close();
                        }
                        else{
                            $sql="DELETE FROM `installment_procurement` WHERE `Qtr_ID`='".$qtr_id."'";
                            $con->query($sql);
                            $stmt=$con->prepare("INSERT INTO `installment_procurement`(`Qtr_ID`,`Installment`) VALUES (?,?)");
                            $stmt->bind_param("si",$qtr_id,$months);
                            $stmt->execute();
